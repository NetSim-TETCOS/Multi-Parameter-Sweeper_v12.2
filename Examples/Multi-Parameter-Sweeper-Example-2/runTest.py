...
NETSIM_PATH="C:\\Users\\TETCOS\\Documents\\NetSim_12.2.26_64_pro_default\\bin\\bin_x64"
...
csvfile.write('CHANNELBANDWIDTH_MHz,DOWNLINK_MIMO_LAYER_COUNT,INTER_ARRIVAL_TIME(micro sec),THROUGHPUT(Mbps),Data Packets transmitted,')
csvfile.close()

k=0
for i in bandwidth:
    k+=1
    for j in dl_mimo_count:
        ...
        cmd='ConfigWriter.exe '+str(i)+' '+str(j)+' '+str(iat)+' '+str(prb_count[k-1])\
        +' '+str(guard_band[k-1])
        print(cmd)
        os.system(cmd)
        ...
        if(os.path.isfile("Metrics.xml")):
            shutil.copy("Metrics.xml", "Data\metrics_"+str(i)+'_'+str(j)+".xml")
